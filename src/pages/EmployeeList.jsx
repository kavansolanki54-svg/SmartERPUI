...
const EmployeeList = () => {
  ...
  const filteredEmployees = employees.filter(e => {
    ...
    const role = String(e.roleName || '').toLowerCase();
    ...
    <>
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100%', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ fontSize: '18px', fontWeight: '700', color: 'var(--color-on-surface)' }}>Employee Master</div>
          <Link
            to="/employee/create"
            style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <input
              ...
              placeholder="Code, name, email or role..."
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border-structural)', backgroundColor: '#f8f9fa' }}>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Code</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Name</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Role</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="4" style={{ padding: '32px', textAlign: 'center', color: 'var(--color-primary)' }}>
                    <i className="fa-solid fa-circle-notch fa-spin fa-2x"></i>
                  </td>
                </tr>
              ) : currentEntries.length > 0 ? currentEntries.map((emp) => {
                const name = emp.employeeName || '-';
                const role = emp.roleName || '-';
                return (
                  <tr key={emp.employeeId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                    <td style={{ padding: '12px 16px', color: 'var(--color-on-surface-variant)', fontWeight: '500' }}>{emp.employeeCode || '-'}</td>
                    <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)' }}>{name}</td>
                    <td style={{ padding: '12px 16px', color: 'var(--color-on-surface-variant)' }}>{role}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Link to={`/employee/edit/${emp.employeeId}`} style={{ backgroundColor: '#f1f5f9', color: '#0ea5e9', border: '1px solid #e2e8f0', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}>
                          <i className="fa-solid fa-pencil" style={{ fontSize: '12px' }}></i>
                        </Link>
                        <button onClick={() => handleDelete(emp.employeeId)} style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fee2e2', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <tr>
                  <td colSpan="4" style={{ padding: '16px', textAlign: 'center', color: 'var(--color-outline)' }}>No employees found</td>
                </tr>
              )}
            </tbody>
          </table>
