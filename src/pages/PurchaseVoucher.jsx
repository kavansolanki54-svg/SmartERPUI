...
import { getLedgers, getGroups, getVoucherConfigurations, savePurchaseVoucher, getPurchaseVoucherById, getLedgerCurrentBalance, getNextVoucherNumber } from '../services/api';

// Simplified and Detailed Purchase Voucher
const PurchaseVoucher = () => {
  ...
  const [purchaseVoucherConfigId, setPurchaseVoucherConfigId] = useState(null);
  ...
  }, [navigate, voucherData, items, purchaseVoucherConfigId]);
  ...
  const fetchData = async () => {
    try {
      ...
      // Find the Voucher Config for "Purchase" (VoucherCategory === 48)
      const purchaseConfig = configsList.find(c => c.voucherCategory === 48);
      let configId = null;
      if (purchaseConfig) {
        configId = purchaseConfig.voucherConfigId;
        setPurchaseVoucherConfigId(purchaseConfig.voucherConfigId);
        setIsAutoNumbering(purchaseConfig.methodOfNumbering === 1);
      } else if (configsList.length > 0) {
        configId = configsList[0].voucherConfigId;
        setPurchaseVoucherConfigId(configsList[0].voucherConfigId);
      ...
      if (id) {
        const voucherRes = await getPurchaseVoucherById(id);
        if (voucherRes.success && voucherRes.data) {
          ...
          // Map LedgerEntries (excluding the Party Credit entry) to Items
          ...
          if (particularEntries.length > 0) {
            const loadedItems = particularEntries.map(e => {
              ...
              return {
                id: e.purchaseVoucherLedgerEntryId || Date.now() + Math.random(),
  ...
  const submitVoucher = async (statusId) => {
    if (!purchaseVoucherConfigId) {
      toast.error("Purchase Voucher Configuration not found!");
      return;
    }
    ...
    try {
      ...
      // 1. Credit the Party for the total amount
      ledgerEntries.push({
      ...
      // 2. Debit the Particulars for their respective amounts
      ...
      const payload = {
        purchaseVoucherHeaderId: id ? parseInt(id, 10) : 0,
        voucherConfigId: purchaseVoucherConfigId,
      ...
      const res = await savePurchaseVoucher(payload);
      if (res.success) {
    ...
    <div style={{ backgroundColor: '#f1f6f8', height: 'calc(100vh - 80px)', overflow: 'hidden', padding: '12px', fontFamily: "'Segoe UI', Consolas, Monaco, monospace", display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          ...
          {/* Supplier Ref and Date info aligned horizontally */}
          <div style={{
            display: 'flex',
            gap: '24px',
            fontSize: '13px',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontWeight: 'bold', color: '#333' }}>Supplier Invoice No:</span>
              <input
                type="text"
