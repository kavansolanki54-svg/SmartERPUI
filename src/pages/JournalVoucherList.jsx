...
const JournalVoucherList = () => {
  ...
  const fetchVouchers = async () => {
    setLoading(true);
    try {
      const response = await getJournalVouchers();
      if (response.success && response.data) {
        setVouchers(response.data);
      } else {
        toast.error(response.message || 'Failed to load journal vouchers');
      }
    } catch (error) {
      console.error('Failed to fetch journal vouchers', error);
      toast.error('An error occurred while loading journal vouchers');
    } finally {
      setLoading(false);
    }
  };
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (res.success) {
                  ...
                  toast.error(res.message || 'Failed to delete journal voucher');
                }
              } catch (err) {
                console.error(err);
  ...
  const filteredVouchers = vouchers.filter(v => {
    ...
    return vNo.includes(searchTerm.toLowerCase()) ||
           ...
           narration.includes(searchTerm.toLowerCase());
  });
  ...
  const renderStatus = (statusId) => {
    if (statusId === 2) return <span style={{ color: '#16a34a', backgroundColor: '#dcfce7', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: '700' }}>POSTED</span>;
    if (statusId === 3) return <span style={{ color: '#ef4444', backgroundColor: '#fee2e2', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: '700' }}>CANCELLED</span>;
  ...
  const colors = { primaryTeal: 'var(--color-primary)' };
    ...
    <>
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100%', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ fontSize: '18px', fontWeight: '700', color: 'var(--color-on-surface)' }}>Journal Voucher</div>
          <Link
            to="/Journal/create"
            style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <input
              ...
              placeholder="Voucher No, Date, Narration..."
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border-structural)', backgroundColor: '#f8f9fa' }}>
                ...
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Narration</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Total Debit</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="6" style={{ padding: '32px', textAlign: 'center', color: colors.primaryTeal }}>
                ...
                return (
                  <tr key={v.journalVoucherHeaderId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                    ...
                    <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)', maxWidth: '240px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.narration || '-'}</td>
                    <td style={{ padding: '12px 16px', color: 'var(--color-on-surface-variant)', fontWeight: '600' }}>₹ {parseFloat(v.totalDebitAmount || 0).toFixed(2)}</td>
                    <td style={{ padding: '12px 16px' }}>{renderStatus(v.voucherStatus)}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Link to={`/Journal/edit/${v.journalVoucherHeaderId}`} style={{ backgroundColor: '#f1f5f9', color: '#0ea5e9', border: '1px solid #e2e8f0', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}>
                          <i className="fa-solid fa-pencil" style={{ fontSize: '12px' }}></i>
                        </Link>
                        <button onClick={() => handleDelete(v.journalVoucherHeaderId)} style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fee2e2', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <tr>
                  <td colSpan="6" style={{ padding: '32px', textAlign: 'center', color: 'var(--color-outline)' }}>
                    <i className="fa-solid fa-book" style={{ fontSize: '32px', opacity: 0.3, display: 'block', marginBottom: '8px' }}></i>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button key={page} onClick={() => handlePageChange(page)}
                  style={{ padding: '6px 12px', backgroundColor: currentPage === page ? colors.primaryTeal : '#fff', border: `1px solid ${currentPage === page ? colors.primaryTeal : 'var(--color-border-structural)'}`, borderRadius: '4px', color: currentPage === page ? '#fff' : 'var(--color-on-surface-variant)', cursor: 'pointer' }}>
                  {page}
                </button>
              ))}
