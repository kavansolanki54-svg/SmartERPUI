...
const LedgerList = () => {
  ...
  const filteredLedgers = ledgers.filter(l =>
    (l.ledgerName && l.ledgerName.toLowerCase().includes(searchTerm.toLowerCase())) ||
    (l.groupName && l.groupName.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  const totalPages = Math.ceil(filteredLedgers.length / entriesPerPage);
  const currentEntries = filteredLedgers.slice((currentPage - 1) * entriesPerPage, currentPage * entriesPerPage);
      ...
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100%', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ fontSize: '14px', fontWeight: '500' }}>
            <span style={{ color: 'var(--color-primary)' }}>Ledger Master</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border-structural)', backgroundColor: '#f8f9fa' }}>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Ledger Name</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Group</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Opening Bal</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Type</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="6" style={{ padding: '32px', textAlign: 'center', color: 'var(--color-primary)' }}>
                    <i className="fa-solid fa-circle-notch fa-spin fa-2x"></i>
                  </td>
                </tr>
              ) : currentEntries.length > 0 ? currentEntries.map((ledger) => (
                <tr key={ledger.ledgerId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                  <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)' }}>{ledger.ledgerName}</td>
                  <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)' }}>{ledger.groupName || '-'}</td>
                  <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)' }}>{ledger.openingBalance}</td>
                ...
                <tr>
                  <td colSpan="6" style={{ padding: '16px', textAlign: 'center', color: 'var(--color-outline)' }}>No ledgers found</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px' }}>
          <div style={{ fontSize: '13px', color: 'var(--color-on-surface-variant)' }}>
            Showing {filteredLedgers.length === 0 ? 0 : (currentPage - 1) * entriesPerPage + 1} to {Math.min(currentPage * entriesPerPage, filteredLedgers.length)} of {filteredLedgers.length} entries
          </div>
