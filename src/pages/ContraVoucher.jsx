...
import { getLedgers, getGroups, getVoucherConfigurations, saveContraVoucher, getContraVoucherById, getLedgerCurrentBalance, getNextVoucherNumber } from '../services/api';

const ContraVoucher = () => {
  const { id } = useParams();
  const navigate = useNavigate();

  const [cashBankLedgers, setCashBankLedgers] = useState([]);   // Both From & To sides are Cash/Bank
  const [contraVoucherConfigId, setContraVoucherConfigId] = useState(null);
  ...
  const [voucherData, setVoucherData] = useState({
    ...
    accountLedgerId: null,   // "To" account (credited/outgoing)
    narration: '',
    referenceNo: '',
  ...
  }, [navigate, voucherData, items, contraVoucherConfigId]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (contraConfig) {
        ...
        setContraVoucherConfigId(contraConfig.voucherConfigId);
        setIsAutoNumbering(contraConfig.methodOfNumbering === 1);
      } else if (configsList.length > 0) {
        const fallback = configsList.find(c => (c.voucherName || '').toLowerCase().includes('contra')) || configsList[0];
        configId = fallback.voucherConfigId;
        setContraVoucherConfigId(fallback.voucherConfigId);
      ...
      cbLedgers.sort((a, b) => a.label.localeCompare(b.label));
      setCashBankLedgers(cbLedgers);

      // If editing
      if (id) {
        const voucherRes = await getContraVoucherById(id);
        if (voucherRes.success && voucherRes.data) {
          const vData = voucherRes.data;

          const accId = vData.accountLedgerId;
          const matchingAccount = cbLedgers.find(p => p.value === accId) || null;
          ...
          // For Contra, the "To" side is the accountLedgerId (Credit, type 2)
          // The "From" (Particulars/Debit, type 1) entries come from ledgerEntries
          const particularEntries = (vData.ledgerEntries || []).filter(e => e.ledgerEntryType === 1); // Debit = From
          if (particularEntries.length > 0) {
            const loadedItems = await Promise.all(particularEntries.map(async (e) => {
              const matchingParticular = cbLedgers.find(p => p.value === e.ledgerId) || null;
              let bal = 0;
              let balType = 'Dr';
              ...
              return {
                id: e.contraVoucherLedgerEntryId || Date.now() + Math.random(),
  ...
  const submitVoucher = async (statusId) => {
    if (!contraVoucherConfigId) {
      toast.error('Contra Voucher Configuration not found!');
      return;
    }
    if (!voucherData.accountLedgerId) {
      toast.error('Please select a Cash/Bank Account (To)!');
      return;
    }
    ...
    try {
      ...
      // Contra: Credit the "To" Cash/Bank account (money going out from this account)
      ledgerEntries.push({
      ...
      // Debit the "From" Cash/Bank accounts (money coming into these accounts)
      ...
      const payload = {
        contraVoucherHeaderId: id ? parseInt(id, 10) : 0,
        voucherConfigId: contraVoucherConfigId,
      ...
      const res = await saveContraVoucher(payload);
      if (res.success) {
        toast.success(statusId === 1 ? 'Contra saved as draft!' : 'Contra posted successfully!');
        navigate('/Contra');
      } else {
        toast.error(res.message || 'Failed to save contra voucher.');
      }
  ...
  return (
    <div style={{ backgroundColor: '#f1f6f8', height: 'calc(100vh - 80px)', overflow: 'hidden', padding: '12px', fontFamily: "'Segoe UI', Consolas, Monaco, monospace", display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        {/* Contra - Account (To) Selection Block */}
        <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <div style={{ width: '350px' }}>
              <Select
                ...
                onChange={(val) => handleInputChange('accountLedgerId', val)}
                options={cashBankLedgers}
                styles={customSelectStyles}
                placeholder="Select Cash / Bank Account (To)"
          ...
          {/* Reference fields for bank transfers */}
          <div style={{
            display: 'flex',
            gap: '24px',
            padding: '8px 16px',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontWeight: 'bold', color: '#003366' }}>Ref/Chq No:</span>
              <input
                type="text"
