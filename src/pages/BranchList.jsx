...
const BranchList = () => {
  ...
  const filteredBranches = branches.filter(b =>
    (b.branchName && b.branchName.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const currentEntries = filteredBranches.slice((currentPage - 1) * entriesPerPage, currentPage * entriesPerPage);
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border-structural)', backgroundColor: '#f8f9fa' }}>
                ...
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Branch Name</th>
            ...
            <tbody>
              {loading ? (
              ...
              ) : currentEntries.length > 0 ? currentEntries.map((branch) => (
                <tr key={branch.branchId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                  <td style={{ padding: '12px 16px', color: 'var(--color-on-surface-variant)', fontWeight: '500' }}>{branch.branchCode}</td>
                  <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)' }}>{branch.branchName}</td>
                  <td style={{ padding: '12px 16px' }}>{renderStatus(branch.activeStatus)}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <Link to={`/branch/edit/${branch.branchId}`} style={{ backgroundColor: '#f1f5f9', color: '#0ea5e9', border: '1px solid #e2e8f0', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}>
                        <i className="fa-solid fa-pencil" style={{ fontSize: '12px' }}></i>
                      </Link>
                      <button onClick={() => handleDelete(branch.branchId)} style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fee2e2', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <tr>
                  <td colSpan="4" style={{ padding: '16px', textAlign: 'center', color: 'var(--color-outline)' }}>No branches found</td>
                </tr>
              )}
            </tbody>
          </table>
