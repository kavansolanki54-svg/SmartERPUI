...
import { getLedgers, getGroups, getVoucherConfigurations, savePaymentVoucher, getPaymentVoucherById, getLedgerCurrentBalance, getNextVoucherNumber } from '../services/api';
...
const PaymentVoucher = () => {
  ...
  const [paymentVoucherConfigId, setPaymentVoucherConfigId] = useState(null);
  ...
  }, [navigate, voucherData, items, paymentVoucherConfigId]);
  ...
  const fetchData = async () => {
    try {
      ...
      // Find the Voucher Config for "Payment" (VoucherCategory === 50)
      const paymentConfig = configsList.find(c => c.voucherCategory === 50);
      let configId = null;
      if (paymentConfig) {
        configId = paymentConfig.voucherConfigId;
        setPaymentVoucherConfigId(paymentConfig.voucherConfigId);
        setIsAutoNumbering(paymentConfig.methodOfNumbering === 1);
      } else if (configsList.length > 0) {
        const fallback = configsList.find(c => (c.voucherName || '').toLowerCase().includes('payment')) || configsList[0];
        configId = fallback.voucherConfigId;
        setPaymentVoucherConfigId(fallback.voucherConfigId);
      ...
      // If editing
      if (id) {
        const voucherRes = await getPaymentVoucherById(id);
        if (voucherRes.success && voucherRes.data) {
          ...
          const particularEntries = (vData.ledgerEntries || []).filter(e => e.ledgerEntryType === 1); // 1 = Debit
          if (particularEntries.length > 0) {
            const loadedItems = await Promise.all(particularEntries.map(async (e) => {
              ...
              return {
                id: e.paymentVoucherLedgerEntryId || Date.now() + Math.random(),
  ...
  const submitVoucher = async (statusId) => {
    if (!paymentVoucherConfigId) {
      toast.error("Payment Voucher Configuration not found!");
      return;
    }
    ...
    try {
      ...
      // 1. Credit the Cash/Bank account for the total amount in DB
      ledgerEntries.push({
      ...
      // 2. Debit the Particulars for their respective amounts in DB
      ...
      const payload = {
        paymentVoucherHeaderId: id ? parseInt(id, 10) : 0,
        voucherConfigId: paymentVoucherConfigId,
      ...
      const res = await savePaymentVoucher(payload);
      if (res.success) {
        toast.success(statusId === 1 ? "Payment saved as draft!" : "Payment posted successfully!");
        navigate('/Payment');
      } else {
        toast.error(res.message || "Failed to save payment voucher.");
      }
  ...
  const displayAccountBalance = () => {
    ...
    const totalAmount = calculateTotal();
    // Visually treated as DR on Main Section, so entryType = 1 (to add/maintain Dr)
    const updated = getUpdatedBalance(
      { balance: accountBalance.amount, balanceType: accountBalance.type },
      totalAmount,
      2 // For payments, cash balance decreases (Credit/EntryType 2)
    );
  ...
  const displayParticularBalance = (item) => {
    ...
    // Visually treated as CR on sub-sections, so entryType = 1 (since Particulars are Debited in Payment)
    const updated = getUpdatedBalance(
      { balance: item.balance, balanceType: item.balanceType },
      item.amount,
      1 // Particular ledger balance increases (Debit/EntryType 1)
    );
    return formatBalance(updated.amount, updated.type);
  };
