...
const RoleList = () => {
  ...
  const filteredRoles = roles.filter(r =>
    (r.roleName && r.roleName.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  const totalPages = Math.ceil(filteredRoles.length / entriesPerPage);
  const currentEntries = filteredRoles.slice((currentPage - 1) * entriesPerPage, currentPage * entriesPerPage);
      ...
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100%', padding: '24px' }}>
        ...
        <div className="dashboard-header-row" style={{ marginBottom: '16px' }}>
          <div style={{ fontSize: '14px', fontWeight: '500' }}>
            <span style={{ color: 'var(--color-primary)' }}>Role Master</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border-structural)', backgroundColor: '#f8f9fa' }}>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Role Name</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="3" style={{ padding: '32px', textAlign: 'center', color: 'var(--color-primary)' }}>
                    <i className="fa-solid fa-circle-notch fa-spin fa-2x"></i>
                  </td>
                </tr>
              ) : currentEntries.length > 0 ? currentEntries.map((role) => (
                <tr key={role.roleMasterId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                  <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)' }}>{role.roleName}</td>
                  <td style={{ padding: '12px 16px' }}>{renderStatus(role.activeStatus)}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <Link to={`/role/edit/${role.roleMasterId}`} style={{ backgroundColor: '#f1f5f9', color: '#0ea5e9', border: '1px solid #e2e8f0', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}>
                        <i className="fa-solid fa-pencil" style={{ fontSize: '12px' }}></i>
                      </Link>
                      <button onClick={() => handleDelete(role.roleMasterId)} style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fee2e2', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <tr>
                  <td colSpan="3" style={{ padding: '16px', textAlign: 'center', color: 'var(--color-outline)' }}>No roles found</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px' }}>
          <div style={{ fontSize: '13px', color: 'var(--color-on-surface-variant)' }}>
            Showing {filteredRoles.length === 0 ? 0 : (currentPage - 1) * entriesPerPage + 1} to {Math.min(currentPage * entriesPerPage, filteredRoles.length)} of {filteredRoles.length} entries
          </div>
