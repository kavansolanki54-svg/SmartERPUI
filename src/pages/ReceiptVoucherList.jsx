...
const ReceiptVoucherList = () => {
  ...
  const fetchVouchers = async () => {
    setLoading(true);
    try {
      const response = await getReceiptVouchers();
      if (response.success && response.data) {
        setVouchers(response.data);
      } else {
        toast.error(response.message || "Failed to load receipt vouchers");
      }
    } catch (error) {
      console.error("Failed to fetch receipt vouchers", error);
      toast.error("An error occurred while loading receipt vouchers");
    } finally {
      setLoading(false);
    }
  };
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (res.success) {
                  ...
                  toast.error(res.message || 'Failed to delete receipt voucher');
                }
              } catch (err) {
                console.error(err);
    ...
    <>
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100%', padding: '24px' }}>
        ...
        <div className="dashboard-header-row" style={{ marginBottom: '16px' }}>
          <div style={{ fontSize: '18px', fontWeight: '700', color: 'var(--color-on-surface)' }}>Receipt Voucher</div>
          <Link
            to="/Receipt/create"
            style={{
          ...
          >
            <i className="fa-solid fa-plus"></i> Add Receipt Voucher
          </Link>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={v.receiptVoucherHeaderId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Link to={`/Receipt/edit/${v.receiptVoucherHeaderId}`} style={{ backgroundColor: '#f1f5f9', color: '#0ea5e9', border: '1px solid #e2e8f0', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}>
                          <i className="fa-solid fa-pencil" style={{ fontSize: '12px' }}></i>
                        </Link>
                        <button onClick={() => handleDelete(v.receiptVoucherHeaderId)} style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fee2e2', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <tr>
                  <td colSpan="6" style={{ padding: '16px', textAlign: 'center', color: 'var(--color-outline)' }}>No receipt vouchers found</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button
                  ...
                  style={{
                    padding: '6px 12px',
                    backgroundColor: currentPage === page ? colors.primaryTeal : '#fff',
                    border: `1px solid ${currentPage === page ? colors.primaryTeal : 'var(--color-border-structural)'}`,
                    borderRadius: '4px',
