...
const COLUMNS = [
  ...
  { key: 'accountName', label: 'ACCOUNT (DEBIT)', align: 'left', sortable: true, filterable: true, filterType: 'text', width: '220px' },
...
const ReceiptVoucherList = () => {
  ...
  const fetchVouchers = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error('Error loading receipt vouchers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = (id) => {
    toast((t) => (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <span style={{ fontSize: '14px', fontWeight: '500' }}>Delete this receipt voucher?</span>
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          ...
          <button onClick={async () => {
            ...
            try {
              ...
              toast.error('Error deleting receipt voucher');
            }
  ...
  const enrichedData = useMemo(() => {
    return vouchers.map(v => ({
      ...
      accountName: v.accountLedgerName || v.cashBankLedgerName || '-',
  ...
  return (
    <div className="list-page-container">
      <EnterpriseDataGrid
        ...
        data={enrichedData}
        loading={loading}
        createLink="/Receipt/create"
        createLabel="Add Receipt Voucher"
        editLinkPrefix="/Receipt/edit"
        onDelete={handleDelete}
        idKey="receiptVoucherHeaderId"
