...
import { getContraVouchers, deleteContraVoucher } from '../services/api';
...
const ContraVoucherList = () => {
  ...
  const fetchVouchers = async () => {
    setLoading(true);
    try {
      const response = await getContraVouchers();
      if (response.success && response.data) {
        setVouchers(response.data);
      } else {
        toast.error(response.message || 'Failed to load contra vouchers');
      }
    } catch (error) {
      console.error('Failed to fetch contra vouchers', error);
      toast.error('An error occurred while loading contra vouchers');
    } finally {
      setLoading(false);
    }
  };
    ...
    toast((t) => (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <span style={{ fontSize: '14px', fontWeight: '500', color: 'var(--color-on-surface)' }}>
          Are you sure you want to delete this contra voucher?
        </span>
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          ...
          <button
            onClick={async () => {
              toast.dismiss(t.id);
              try {
                const res = await deleteContraVoucher(id);
                if (res.success) {
                  toast.success('Contra voucher deleted successfully');
                  fetchVouchers();
                } else {
                  toast.error(res.message || 'Failed to delete contra voucher');
                }
              } catch (err) {
                console.error(err);
  ...
  const filteredVouchers = vouchers.filter(v => {
    ...
    const account = String(v.accountLedgerName || '').toLowerCase();
    ...
    <>
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100%', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ fontSize: '18px', fontWeight: '700', color: 'var(--color-on-surface)' }}>Contra Voucher</div>
          <Link
            to="/Contra/create"
            style={{
          ...
          >
            <i className="fa-solid fa-plus"></i> Add Contra Voucher
          </Link>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <input
              ...
              placeholder="Voucher No, Date, Account..."
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border-structural)', backgroundColor: '#f8f9fa' }}>
                ...
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: '600', color: 'var(--color-on-surface)' }}>Cash/Bank Account</th>
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={v.contraVoucherHeaderId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                    ...
                    <td style={{ padding: '12px 16px', color: 'var(--color-on-surface)' }}>{v.accountLedgerName || '-'}</td>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Link to={`/Contra/edit/${v.contraVoucherHeaderId}`} style={{ backgroundColor: '#f1f5f9', color: '#0ea5e9', border: '1px solid #e2e8f0', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}>
                          <i className="fa-solid fa-pencil" style={{ fontSize: '12px' }}></i>
                        </Link>
                        <button onClick={() => handleDelete(v.contraVoucherHeaderId)} style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fee2e2', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <tr>
                  <td colSpan="6" style={{ padding: '32px', textAlign: 'center', color: 'var(--color-outline)' }}>
                    <i className="fa-solid fa-arrow-right-arrow-left" style={{ fontSize: '32px', opacity: 0.3, display: 'block', marginBottom: '8px' }}></i>
