  ...
  const fetchVouchers = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast.error(response.message || "Failed to load payment vouchers");
      }
    } catch (error) {
      console.error("Failed to fetch payment vouchers", error);
      toast.error("An error occurred while loading payment vouchers");
    } finally {
      setLoading(false);
    }
  };
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (res.success) {
                  ...
                  toast.error(res.message || 'Failed to delete payment voucher');
                }
              } catch (err) {
                console.error(err);
    ...
    <>
      <div style={{ backgroundColor: '#f8f9fa', minHeight: '100%', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          >
            <i className="fa-solid fa-plus"></i> Add Payment Voucher
          </Link>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={v.paymentVoucherHeaderId} style={{ borderBottom: '1px solid var(--color-border-structural)' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Link to={`/Payment/edit/${v.paymentVoucherHeaderId}`} style={{ backgroundColor: '#f1f5f9', color: '#0ea5e9', border: '1px solid #e2e8f0', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', textDecoration: 'none' }}>
                          <i className="fa-solid fa-pencil" style={{ fontSize: '12px' }}></i>
                        </Link>
                        <button onClick={() => handleDelete(v.paymentVoucherHeaderId)} style={{ backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fee2e2', borderRadius: '4px', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <tr>
                  <td colSpan="6" style={{ padding: '16px', textAlign: 'center', color: 'var(--color-outline)' }}>No payment vouchers found</td>
                </tr>
              )}
            </tbody>
          </table>
