...
const COLUMNS = [
  ...
  { key: 'partyName', label: 'PARTY / SUPPLIER', align: 'left', sortable: true, filterable: true, filterType: 'text', width: '220px' },
...
const PurchaseVoucherList = () => {
  ...
  const fetchVouchers = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error('Error loading purchase vouchers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = (id) => {
    toast((t) => (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <span style={{ fontSize: '14px', fontWeight: '500' }}>Delete this purchase voucher?</span>
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          ...
          <button onClick={async () => {
            ...
            try {
              ...
              toast.error('Error deleting purchase voucher');
            }
  ...
  const enrichedData = useMemo(() => {
    return vouchers.map(v => ({
      ...
      partyName: v.partyLedgerName || v.supplierLedgerName || '-',
  ...
  return (
    <div className="list-page-container">
      <EnterpriseDataGrid
        ...
        createLabel="Add Purchase Voucher"
        editLinkPrefix="/Purchase/edit"
        onDelete={handleDelete}
        idKey="purchaseVoucherHeaderId"
